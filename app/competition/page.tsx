...
const competitionSchedule = [
  ...
  { month: "10月", dates: "4日(土) - 5日(日)", title: "第323回 日本体育大学長距離競技会 兼 第17回NCG", isNCG: true, note: "※競技場改修工事の関係で変更になる可能性があります" },
    ...
    <div className="bg-gradient-to-b from-white to-sky-50 min-h-screen">
      ...
      >
        <div className="max-w-4xl mx-auto">
          ...
          <Alert className="mb-8 border-sky-600 bg-sky-50 shadow-sm">
            <Info className="h-5 w-5 text-sky-600" />
            <AlertTitle className="text-sky-800 font-bold">お知らせ</AlertTitle>
            <AlertDescription className="text-sky-800">
              大会関係資料をよく読むようにお願いいたします。今年度の競技会注意事項をよく読むようお願いいたします。
            </AlertDescription>
          </Alert>
            ...
            <InfoCard
              ...
              content={
                <div className="space-y-2">
                  ...
                  />
                  <DocumentLink 
                    href="https://nssu-ekiden.com/wp-content/uploads/2025/03/長距離記録証の発行に関して.pdf"
                    title="記録証の発行について"
                  />
                </div>
              }
              ...
              content={
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="mb-4">
                      団体登録・エントリーは以下のサイトから行ってください。
                    </p>
                    <Button className="mt-2 bg-sky-600 hover:bg-sky-700" asChild>
                      <a href="http://ld.nssu-athletic.com/" target="_blank" rel="noopener noreferrer">
                        エントリーサイトへ <ChevronRight className="ml-2 h-4 w-4" />
                      </a>
                    </Button>
                  </div>
                </div>
...
function ScheduleItem({ month, dates, title, note }: ScheduleItemProps) {
  return (
    <div className="p-4 hover:bg-sky-50 transition-colors flex flex-col sm:flex-row items-start">
      ...
      <div className="flex-1">
        ...
        {note && (
          <p className="text-sm text-amber-600 mt-1">{note}</p>
        )}
      </div>
    </div>
  );
