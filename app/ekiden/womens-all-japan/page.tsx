...
// --- データ読み込み ---
const loadWomensAllJapanHistory = async (): Promise<HistoryEntry[]> => {
    try {
        const response = await fetch('/data/ekiden/womens-all-japan-history.json');
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
        const historyData = data['womens-all-japan-history'];
        ...
        console.error('Failed to load womens-all-japan-history data:', error);
        return [];
    }
};
                    ...
                    <AccordionTrigger className="p-4 hover:bg-gray-50/50">
                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center w-full gap-2">
                            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                                <span className="text-2xl font-bold text-pink-800 w-16 text-center">{entry.rank}位</span>
                            ...
                            <div className="text-left sm:text-right">
                                ...
                                <p className="text-xl font-bold font-mono text-pink-900">{entry.totalTime}</p>
                            </div>
                        </div>
                    </AccordionTrigger>
                                ...
                                {entry.runners && entry.runners.map(runner => (
                                    <TableRow key={runner.section}>
                                        ...
                                        <TableCell>
                                            ...
                                            {runner.grade && <span className="text-xs text-gray-500 ml-1">({runner.grade}年)</span>}
                                        </TableCell>
                                        <TableCell className="text-center">{runner.rank}位<RankIcon rank={runner.rank} /></TableCell>
                                        <TableCell className="text-right font-mono">
                                            {runner.time}
                                            {runner.isSectionRecord && <span title="区間記録"><Star className="h-4 w-4 text-pink-500 fill-pink-500 inline-block ml-1" /></span>}
                                        </TableCell>
                                    </TableRow>
                                ))}
                            </TableBody>
...
export default function WomensEkidenHistoryPage() {
    const [womensAllJapanHistory, setWomensAllJapanHistory] = useState<HistoryEntry[]>([]);
    ...
    useEffect(() => {
        const loadData = async () => {
            setLoading(true);
            const data = await loadWomensAllJapanHistory();
            setWomensAllJapanHistory(data);
            if (data.length > 0) {
    ...
    const handleCompare = () => {
        const dataA = womensAllJapanHistory.find(entry => entry.kai === parseInt(compareA));
        const dataB = womensAllJapanHistory.find(entry => entry.kai === parseInt(compareB));
        if (dataA && dataB) {
    ...
    const filteredHistory = useMemo(() => {
        ...
        if (!debouncedSearchTerm) return womensAllJapanHistory;
        return womensAllJapanHistory.filter(entry =>
            entry.runners && entry.runners.some(runner =>
                runner.name.toLowerCase().includes(debouncedSearchTerm.toLowerCase())
            )
        );
    }, [debouncedSearchTerm, womensAllJapanHistory]);
    ...
    return (
        <motion.div 
            ...
            className="min-h-screen bg-gradient-to-br from-white to-pink-50"
        >
            <div className="container mx-auto px-4 py-8">
                <div className="mb-6">
                    <Breadcrumbs items={[{ label: 'ホーム', href: '/' }, { label: '駅伝', href: '/ekiden' }, { label: '全日本大学女子駅伝' }]} />
                </div>
                <AnimatedPageHeader
                    title="全日本大学女子駅伝"
                    subtitle="女子駅伝ブロック"
                ...
                {comparisonResult && (
                    <Card className="mb-6">
                        <CardHeader>
                            <CardTitle className="text-xl font-bold text-pink-800">比較結果</CardTitle>
                        </CardHeader>
                        <CardContent>
                    ...
                    <CardContent className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-pink-50 rounded-lg">
                            <div className="space-y-2">
                                <label className="text-sm font-medium text-pink-700">比較対象A</label>
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-medium text-pink-700">比較対象B</label>
                            ...
                            <div className="flex flex-col justify-end space-y-2">
                                <Button onClick={handleCompare} className="bg-pink-600 hover:bg-pink-700">
                ...
                <AnimatePresence>
                    {showScroll && (
                        <motion.button
                            ...
                            onClick={scrollToTop}
                            className="fixed bottom-8 right-8 bg-pink-600 hover:bg-pink-700 text-white p-3 rounded-full shadow-lg transition-colors z-50"
                        >
            ...
            <Table>
                ...
                <TableBody>
                    <TableRow>
                        ...
                        <TableCell className={`text-right ${isFinite(diff) && diff <= 0 ? 'font-bold text-pink-600' : ''}`}>{dataA.totalTime}</TableCell>
                        <TableCell className={`text-right ${isFinite(diff) && diff > 0 ? 'font-bold text-pink-600' : ''}`}>{dataB.totalTime}</TableCell>
                                ...
                                {dataA.runners.map((rA, i) => {
                                    ...
                                    return (
                                        <TableRow key={i}>
                                            ...
                                            <TableCell className={`${isFinite(tDiff) && tDiff <= 0 ? 'font-bold text-pink-600' : ''}`}>{rA.name}<br/><span className="text-xs text-gray-500">{rA.time}</span></TableCell>
                                            <TableCell className={`${isFinite(tDiff) && tDiff > 0 ? 'font-bold text-pink-600' : ''}`}>{rB.name}<br/><span className="text-xs text-gray-500">{rB.time}</span></TableCell>
