...
// --- データ読み込み ---
const loadFujisanHistory = async (): Promise<HistoryEntry[]> => {
    try {
        const response = await fetch('/data/ekiden/fujisan-history.json');
        const data = await response.json();
        return data['fujisan-history'];
    } catch (error) {
        console.error('Failed to load fujisan-history data:', error);
        return [];
    }
};
    ...
    return (
        <Card className={`overflow-hidden transition-shadow hover:shadow-lg ${getRankBorderClass(entry.rank)}`}>
            <Accordion type="single" collapsible>
                <AccordionItem value={`item-${entry.kai}-${entry.year}`} className="border-b-0">
                                ...
                                {entry.runners.map(runner => (
                                    <TableRow key={runner.section}>
                                        ...
                                        <TableCell className="text-right font-mono">
                                            ...
                                            {runner.isSectionNew && <span title="区間新記録"><Star className="h-4 w-4 text-red-500 fill-red-500 inline-block ml-1" /></span>}
                                        </TableCell>
                                    </TableRow>
                                ))}
                            </TableBody>
...
export default function FujiWomensEkidenHistoryPage() {
    const [fujiHistory, setFujiHistory] = useState<HistoryEntry[]>([]);
    ...
    useEffect(() => {
        const loadData = async () => {
            setLoading(true);
            const data = await loadFujisanHistory();
            setFujiHistory(data);
            if (data.length > 0) {
    ...
    const filteredHistory = useMemo(() => {
        if (!fujiHistory || fujiHistory.length === 0) return [];
        if (!debouncedSearchTerm) return fujiHistory;
        return fujiHistory.filter(entry =>
            entry.runners && entry.runners.some(runner =>
                runner.name.toLowerCase().includes(debouncedSearchTerm.toLowerCase())
            )
        );
    }, [debouncedSearchTerm, fujiHistory]);

    const handleCompare = () => {
        const dataA = fujiHistory.find(entry => entry.kai === parseInt(compareA));
        const dataB = fujiHistory.find(entry => entry.kai === parseInt(compareB));
        if (dataA && dataB) {
    ...
    const validEntries = useMemo(() => {
        if (!fujiHistory || fujiHistory.length === 0) return [];
        return fujiHistory.filter(entry => entry && entry.kai);
    }, [fujiHistory]);
        ...
        >
            <div className="container mx-auto px-4 py-8">
                ...
                {comparisonResult && (
                    <Card className="mb-6">
                        <CardHeader>
                            <CardTitle className="text-xl font-bold text-orange-800">比較結果</CardTitle>
                        </CardHeader>
                        <CardContent>
                    ...
                    <CardContent className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-orange-50 rounded-lg">
                            <div className="space-y-2">
                                <label className="text-sm font-medium text-orange-700">比較対象A</label>
                                <Select value={compareA} onValueChange={setCompareA}>
                                    ...
                                    <SelectContent>
                                        {validEntries.map(entry => (
                                            <SelectItem key={`${entry.kai}-${entry.year}`} value={String(entry.kai)}>
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-medium text-orange-700">比較対象B</label>
                            ...
                            <div className="flex flex-col justify-end space-y-2">
                                <Button onClick={handleCompare} className="bg-orange-600 hover:bg-orange-700">
                ...
                <AnimatePresence>
                    {showScroll && (
                        <motion.button
                            ...
                            onClick={scrollToTop}
                            className="fixed bottom-8 right-8 bg-orange-600 hover:bg-orange-700 text-white p-3 rounded-full shadow-lg transition-colors z-50"
                        >
