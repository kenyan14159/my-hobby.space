...
import { ArrowUp, Award, Medal, Search, Trophy, X, Star, Users, Scale } from "lucide-react";
...
// --- データ読み込み ---
const loadMixedHistory = async (): Promise<HistoryEntry[]> => {
    try {
        const response = await fetch('/data/ekiden/mixed-history.json');
        const data = await response.json();
        return data['mixed-history'];
    } catch (error) {
        console.error('Failed to load mixed-history data:', error);
        return [];
    }
};

// --- ヘルパー関数 ---
const getGenderColor = (gender: "男子" | "女子") => {
    return gender === "男子" ? "text-blue-600" : "text-pink-600";
};
...
const HistoryCard = ({ entry, searchTerm }: { entry: HistoryEntry, searchTerm: string }) => {
    const getRankBorderClass = (rank: number) => {
        if (rank === 1) return 'border-l-4 border-amber-400';
        if (rank === 2) return 'border-l-4 border-slate-400';
    ...
    return (
        <Card className={`overflow-hidden transition-shadow hover:shadow-lg ${getRankBorderClass(entry.rank)}`}>
            <Accordion type="single" collapsible>
                <AccordionItem value={`item-${entry.year}`} className="border-b-0">
                    <AccordionTrigger className="p-4 hover:bg-gray-50/50">
                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center w-full gap-2">
                            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                                <span className="text-2xl font-bold text-indigo-800 w-16 text-center">{entry.rank}位</span>
                                <div className="text-left">
                                    <h3 className="text-lg font-semibold text-gray-800">
                                        {entry.year}年 {entry.tournamentName}
                                        {entry.isTournamentRecord && (
                                            <span title="大会記録" className="inline-flex items-center bg-indigo-100 text-indigo-700 text-xs font-bold px-2 py-1 rounded-full ml-2">
                            ...
                            <div className="text-left sm:text-right">
                                ...
                                <p className="text-xl font-bold font-mono text-indigo-900">{entry.totalTime}</p>
                            </div>
                        </div>
                    </AccordionTrigger>
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow>
                                    ...
                                    <TableHead className="text-center">距離</TableHead>
                            ...
                            <TableBody>
                                {entry.runners.map(runner => (
                                    <TableRow key={runner.section}>
                                        ...
                                        <TableCell className={`text-center font-medium ${getGenderColor(runner.gender)}`}>
                                            {runner.gender}
                                        </TableCell>
                                        <TableCell className="text-center text-sm text-gray-600">{runner.distance}</TableCell>
                                        <TableCell className="text-center">
                                            {runner.rank}
                                            <RankIcon rank={runner.rank} />
                                        </TableCell>
                                        <TableCell className="text-right font-mono">
                                            {runner.time}
                                            {runner.isSectionRecord && <span title="区間記録"><Star className="h-4 w-4 text-indigo-500 fill-indigo-500 inline-block ml-1" /></span>}
                                        </TableCell>
                                    </TableRow>
                                ))}
                            </TableBody>
...
export default function MixedEkidenHistoryPage() {
    const [mixedHistory, setMixedHistory] = useState<HistoryEntry[]>([]);
    ...
    useEffect(() => {
        const loadData = async () => {
            setLoading(true);
            const data = await loadMixedHistory();
            setMixedHistory(data);
            if (data.length > 0) {
                setCompareA(String(data[0].year));
                setCompareB(String(data[1]?.year || data[0].year));
            }
            setLoading(false);
        };
        loadData();
    ...
    const handleCompare = () => {
        const dataA = mixedHistory.find(entry => entry.year === parseInt(compareA));
        const dataB = mixedHistory.find(entry => entry.year === parseInt(compareB));
        if (dataA && dataB) {
    ...
    const filteredHistory = useMemo(() => {
        if (!mixedHistory || mixedHistory.length === 0) return [];
        if (!debouncedSearchTerm) return mixedHistory;
        return mixedHistory.filter(entry =>
            entry.runners && entry.runners.some(runner =>
                runner.name.toLowerCase().includes(debouncedSearchTerm.toLowerCase())
            )
        );
    }, [debouncedSearchTerm, mixedHistory]);

    const validEntries = useMemo(() => {
        if (!mixedHistory || mixedHistory.length === 0) return [];
        return mixedHistory.filter(entry => entry && entry.year);
    }, [mixedHistory]);
    ...
    return (
        <motion.div 
            ...
            className="min-h-screen bg-gradient-to-br from-white to-indigo-50"
        >
            <div className="container mx-auto px-4 py-8">
                <div className="mb-6">
                    <Breadcrumbs items={[{ label: 'ホーム', href: '/' }, { label: '駅伝', href: '/ekiden' }, { label: '男女混合駅伝' }]} />
                </div>
                <AnimatedPageHeader
                    title="男女混合駅伝"
                ...
                {comparisonResult && (
                    <Card className="mb-6">
                        <CardHeader>
                            <CardTitle className="text-xl font-bold text-indigo-800">比較結果</CardTitle>
                        </CardHeader>
                        <CardContent>
                    ...
                    <CardContent className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-indigo-50 rounded-lg">
                            <div className="space-y-2">
                                <label className="text-sm font-medium text-indigo-700">比較対象A</label>
                                <select 
                                    value={compareA} 
                                ...
                                >
                                    {validEntries.map(entry => (
                                        <option key={entry.year} value={String(entry.year)}>
                                            {entry.year}年 - {entry.rank}位
                                        </option>
                                    ))}
                                </select>
                            </div>
                            <div className="space-y-2">
                                <label className="text-sm font-medium text-indigo-700">比較対象B</label>
                                <select 
                                    value={compareB} 
    ...
    return (
        <div className="border-t pt-4">
            <p className="text-center font-semibold text-lg mb-2">
                {winner ? `${winner.year}年が ${secondsToTime(Math.abs(diff))} 速い` : 'タイム比較結果'}
            </p>
            <Table>
                <TableHeader>
                    <TableRow>
                        <TableHead>項目</TableHead>
                        <TableHead className="text-right">{dataA.year}年</TableHead>
                        <TableHead className="text-right">{dataB.year}年</TableHead>
                ...
                <TableBody>
                    <TableRow>
                        ...
                        <TableCell className={`text-right ${isFinite(diff) && diff <= 0 ? 'font-bold text-indigo-600' : ''}`}>{dataA.totalTime}</TableCell>
                        <TableCell className={`text-right ${isFinite(diff) && diff > 0 ? 'font-bold text-indigo-600' : ''}`}>{dataB.totalTime}</TableCell>
                        ...
                        <Table>
                           <TableHeader>
                                <TableRow>
                                    ...
                                    <TableHead>{dataA.year}年</TableHead>
                                    <TableHead>{dataB.year}年</TableHead>
                                    <TableHead className="text-right">差</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {dataA.runners.map((rA, i) => {
                                    ...
                                    return (
                                        <TableRow key={i}>
                                            ...
                                            <TableCell className={`${isFinite(tDiff) && tDiff <= 0 ? 'font-bold text-indigo-600' : ''}`}>
                                                {rA.name} ({rA.gender})<br/>
                                                <span className="text-xs text-gray-500">{rA.time}</span>
                                            </TableCell>
                                            <TableCell className={`${isFinite(tDiff) && tDiff > 0 ? 'font-bold text-indigo-600' : ''}`}>
                                                {rB.name} ({rB.gender})<br/>
                                                <span className="text-xs text-gray-500">{rB.time}</span>
                                            </TableCell>
