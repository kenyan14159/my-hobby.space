...
export function Navigation() {
  ...
  return (
    <header className="sticky top-0 z-40 w-full max-w-full bg-white border-b border-sky-100 shadow-sm">
...
function MobileNavigation({ closeMobileMenu }: { closeMobileMenu: () => void }) {
  ...
  return (
    <div className="relative h-screen w-full">
      {/* メニューコンテンツ */}
      <div className="relative z-10 px-6 pt-4 pb-8 h-full flex flex-col">
        {/* メニュー項目 */}
        <div className="flex-1 space-y-6">
          {menuStructure.map((section, index) => (
            <div key={index} className="text-sky-700">
              {section.href ? (
                <div className="flex w-full items-center justify-between">
                  <Link
                    href={section.href}
                    className="py-2 text-sky-700 font-semibold text-lg flex-1 hover:text-sky-500 transition-colors"
                  ...
                  {section.submenu && (
                    <button
                      className="p-2 text-sky-700 hover:text-sky-500 transition-colors"
                ...
                <button
                  className="flex w-full justify-between items-center py-2 text-sky-700 font-semibold text-lg hover:text-sky-500 transition-colors text-left"
                  onClick={() => toggleSection(section.title)}
                  aria-expanded={openSections[section.title] || false}
                >
                  {section.title}
                ...
                <div className="pl-4 pt-2 pb-2 space-y-2">
                  {section.submenu.flatMap(submenu => submenu.items).map((item, itemIndex) => (
                     <li key={itemIndex} className="list-none">
                        <Link
                          href={item.href}
                          className="block py-1.5 text-sky-600 hover:text-sky-700 transition-colors text-base"
                          onClick={handleLinkClick}
                        >
                          {item.title}
                        </Link>
        ...
        <div className="mt-8 mb-16">
          <h4 className="text-sm font-bold mb-3 text-sky-800">SNS</h4>
          <div className="flex items-center space-x-4">
            <a href="https://x.com/nssu_ekiden" target="_blank" rel="noopener noreferrer" aria-label="X" className="text-sky-700 hover:text-sky-500 transition-colors">
              <svg role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 fill-current"><path d="M18.901 1.153h3.68l-8.04 9.19L24 22.846h-7.406l-5.8-7.584-6.638 7.584H.474l8.6-9.83L0 1.154h7.594l5.243 7.184L18.901 1.153zm-1.65 19.54h2.6l-11.2-12.612H7.23l11.2 12.612z"/></svg>
            </a>
            ...
            <a href="https://line.me/R/ti/p/@001cfrru?oat_content=url&ts=02151453" target="_blank" rel="noopener noreferrer" aria-label="LINE" className="text-sky-700 hover:text-sky-500 transition-colors">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50" role="img" className="h-6 w-6 fill-current">
        ...
        <div className="absolute bottom-6 right-6">
          <button
            ...
            className="w-12 h-12 bg-sky-100 rounded-full flex items-center justify-center hover:bg-sky-200 transition-colors"
            aria-label="メニューを閉じる"
          >
